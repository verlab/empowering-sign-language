...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False, instance_name = None):
        import time
        st = time.time()
        self.G.eval()
        ...
        frames_root = "/app/frames_root"
        videos_root = "/app/videos_root"
        os.makedirs(frames_root, exist_ok = True)
        os.makedirs(videos_root, exist_ok = True)

        video_name = instance_name.replace(".npz", "_rendered.mp4")
        video_fp = os.path.join(videos_root, video_name)
        writer = cv2.VideoWriter(video_fp, cv2.VideoWriter_fourcc(*'mp4v'), 15, (256, 256))
        idx = 0
        ...
        for i, frame in enumerate(fls):
            ...
            for i in range(g_out.shape[0]):
                ...
                #here we generate the representative figure for the GRSI
                #we pick here the approximate frames we used to build paper figure 10
                if instance_name == "fZgWKh3ENoE_4-8-rgb_front.npz":
                    frames_idxs = [0, 16, 54, 58, 63]
                    if idx in frames_idxs:
                        rendered_frames.append(frame)
                        landmarks_frames.append(fl_img)

                if instance_name == "fzncPNr2Sc0_3-8-rgb_front.npz":
                    frames_idxs = [8, 21, 53, 61, 63]
                    if idx in frames_idxs:
                        rendered_frames.append(frame)
                        landmarks_frames.append(fl_img)

                if instance_name == "fzDHRCKr7wU_10-8-rgb_front.npz":
                    frames_idxs = [0, 5, 13, 32, 44]
                    if idx in frames_idxs:
                        rendered_frames.append(frame)
                        landmarks_frames.append(fl_img)
        ...
        if instance_name in ["fZgWKh3ENoE_4-8-rgb_front.npz", "fzncPNr2Sc0_3-8-rgb_front.npz", "fzDHRCKr7wU_10-8-rgb_front.npz"]:
            concatenated_rendered_frames = np.hstack((rendered_frames[0], rendered_frames[1], rendered_frames[2], rendered_frames[3], rendered_frames[4]))
            concatenated_landmarks_frames = np.hstack((landmarks_frames[0], landmarks_frames[1], landmarks_frames[2], landmarks_frames[3], landmarks_frames[4]))

        if instance_name == "fZgWKh3ENoE_4-8-rgb_front.npz":
            self.row1 = np.vstack((concatenated_rendered_frames, concatenated_landmarks_frames))

        if instance_name == "fzncPNr2Sc0_3-8-rgb_front.npz":
            self.row2 = np.vstack((concatenated_rendered_frames, concatenated_landmarks_frames))

        if instance_name == "fzDHRCKr7wU_10-8-rgb_front.npz":
            self.row3 = np.vstack((concatenated_rendered_frames, concatenated_landmarks_frames))
