...
def replace_mean_face(instance):
    my_instances_formatted = list()
    mean_face = np.load("mean_face_phoenix_sp4.npy")[:-1, :]
    flag = False
...
def main():
    ...
    #baseline_mt_root = "/home/rafaelv/masters/baselines/makeittalk_out/validation"
    #baseline_pt_root = "/home/rafaelv/masters/baselines/pt2/"
    #baseline_nslp_root = "/srv/storage/datasets/rafaelvieira/rafael_denormalized_outputs_face/signer_8"
    gts_root = "/home/rafael/data/how2sign_dataset_speaker3_clean/test"
    ours_root = "/home/rafael/masters/to_move/outputs_signer03_h2s"
    
    """
    baseline_mt_root = "/home/rafaelv/masters/text2expression/test_kps_ps5_makeittalk/"
    baseline_pt_root = "/home/rafaelv/masters/baselines/pt_ps5/"
    gts_root = "/home/rafaelv/masters/datasets/slp_dataset_f68_full_ps5_new/slp_dataset_f68_full_ps5/val"
    ours_root = "/home/rafaelv/masters/text2expression/test_kps_ps5/"
    """
    ...
    for instance_name in os.listdir(gts_root):
        ...
        try:
            ...
            #pt_instances.append(_read_pt_instance(baseline_pt_root, instance_name.replace(".pkl", ".npy"))[:,:-1].reshape(-1, 68, 2))
            ours_instances.append(_read_ours_instance(ours_root, instance_name.replace(".pkl", ".npz")).squeeze(0)[:, :-1, :])
            #mt_instances.append(_read_makeittalk_instance(baseline_mt_root, instance_name.replace(".pkl", ".npy")))
            #nslp_instances.append(_read_instance_gt(baseline_nslp_root, instance_name).numpy()/256)
            test_names.append(instance_name)
        except:
    ...
    metrics_dict = dict(
        ...
        #pt = (gt_instances, pt_instances),
        #mt = (gt_instances, mt_instances)
        #nslp = (gt_instances, nslp_instances)
    )
    ...
    for key, values in metrics_dict.items():
        ...
        for gt_instance, pred_instance in zip(values[0], values[1]):
            ...
            gt_instance = replace_mean_face(gt_instance)

            if key == "pt":
                pred_instance = np.asarray(sample_frames(pred_instance)[1:])
                pred_instance = replace_mean_face(pred_instance)
            if key == "mt":
                pred_instance = format_kps(pred_instance)
                pred_instance = np.asarray(sample_frames(pred_instance)[1:])
                pred_instance = replace_mean_face(pred_instance)
    ...
    df = df.round(4)
    df.to_csv("metrics_ours_h2s_sp3.csv", sep = ";")
