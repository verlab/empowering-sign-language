...
parser.add_argument('--path_gt', '-pgt', type = str, default = "/srv/storage/datasets/rafaelvieira/thiago_ours")

parser.add_argument('--path_generated', '-pg', type = str, default = "/srv/storage/datasets/rafaelvieira/thiago_gt")

parser.add_argument('--keypoint_type', '-kt', type = str, default = "ours")

parser.add_argument('--path_checkpoints', '-pck', type = str, default = "/srv/storage/datasets/rafaelvieira/checkpoints_fgd_bs_64/lr:0.0005-bs:24-e:500-ts:2022-10-23 14:23:02.795074/step_70080/autoencoder.pth")


def _read_instance(root, instance_name):
...
def compute_statistics_of_path(path, model, batch_size, keypoint_type, device):
    ...
    for instance_name in sorted(os.listdir(path)):
        try:
            if keypoint_type == "gt":
                my_instances.append(np.asarray(_read_instance(path, instance_name)))
            elif keypoint_type == "ours":
                my_instances.append(np.asarray(_read_instance(path, instance_name)))
        except:
...
def calculate_fid_given_paths(paths_gt, paths_g, batch_size, device, keypoint_type,path_checkpoints):
    ...
    model = EmbeddingNet(240, 64).to(device)
